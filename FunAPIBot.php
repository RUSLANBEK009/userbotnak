<?php
define('API_KEY',"6269583367:AAHfrkFZtso_RstfnQmPqdAnH1Wh4-bVSBs");

$builder24 = "5857267647";
...
$balance= json_decode(file_get_contents("https://funapi.uz/api/v2?key=$api&action=balance"),true);
...
bot('sendMessage',[
'chat_id'=>$ccid,
'text'=>"<b>@FunAPIBot dan olingan APIni yuboring:</b>",
'parse_mode'=>"html",
'reply_markup'=>json_encode([
'resize_keyboard'=>true,
'keyboard'=>[
...
$balance= json_decode(file_get_contents("https://funapi.uz/api/v2?key=$text&action=balance"),true);
$valyuta=$balance['currency'];
if($balance['balance']==null){
bot('sendMessage',[
'chat_id'=>$cid,
...
$urll=json_decode(file_get_contents("https://funapi.uz/api/v2?key=$api&action=add&link=$url&quantity=$son&service=$id"),true);
$order=$urll['order'];
if($order==null){
bot('sendMessage',[
'chat_id'=>$ccid,
...
if($byyoq){
if(is_numeric($text)){
$orderstat=json_decode(file_get_contents("https://funapi.uz/api/v2?key=$api&action=status&order=$byyoq"),true);
...
$orderstat=json_decode(file_get_contents("https://funapi.uz/api/v2?key=$api&action=status&order=$byxolat"),true);
$miqdor=$orderstat['remains'];
$xolati=$orderstat['status'];
if($xolati=="Pending"){
$byxol = "Kutilmoqda";
...
$orderstat=json_decode(file_get_contents("https://funapi.uz/api/v2?key=$api&action=status&order=$byid"),true);
$xolati=$orderstat['status'];
$miqdor=$orderstat['remains'];
if($xolati=="Completed"){
bot('sendMessage',[
